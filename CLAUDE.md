...
6. `REVIEW` → `SUBMITTED` — posts the GRPO (per-box tracking concat into `U_pFrtTracking`, per-box freight summed into `U_InboundFrt`). Status flips to SUBMITTED as soon as SAP accepts the receipt, then the photo upload runs **with the receiver still on the screen**, showing a progress bar. Done and Print Picklist stay hidden until it finishes. This is deliberate — see Photo upload below.
...
## Photo upload
- **The upload is awaited, not backgrounded.** It used to be fire-and-forget so the receiver could leave immediately. That doesn't survive a phone: iOS stops the page the moment the screen locks or the app is switched away, and the remaining uploads die silently. GRPO 77197 landed **4 of 14** photos that way, and **3 of the last 4** app receipts lost their photos entirely (`U_GRPODocs` empty, no file cards, no error anywhere). The receipt is already safe in SAP by then; the evidence exists only on the phone until it lands.
- The success card shows a progress bar and "Keep this screen open"; **Done and Print Picklist are hidden until the upload completes**, because printing opens a system sheet and Done leaves the screen — both used to strand the upload.
- Diagnosing a repeat: an empty `OPDN.U_GRPODocs` on a receipt whose `U_GRPOdetails` says "Received via PWA" means the upload never reported success. Cross-check `dbo.file_cards` (no rows) and the proxy log (no `[FileCards]` / `Patching … U_GRPODocs` lines for that user).
- **`GET /api/photo-audit?days=7` reports exactly that**, and the Dashboard shows a banner whenever it finds any. The banner is dismissible **per receipt** (acknowledged GRPO numbers in `localStorage`, per device) — never a blanket mute, because a switch that silences it permanently would rebuild the exact hole this exists to close. Acknowledgements are pruned once a receipt ages out of the audit window. The check reads SAP rather than anything local on purpose: the receipts that lose photos are the ones whose phone stopped mid-upload, so the device that failed can never be the one to report it. Results are cached 3 minutes — the Dashboard asks on every open and the SAP walk is several round trips.
- Scale of the original problem, for reference: **64 of 173 app receipts between 2026-07-06 and 2026-08-27 lost their photos — 37%**, both receivers, unnoticed for two months. It was worse before (43% in July) than after (24% in late August), so it was never a recent regression — it was the original fire-and-forget design.
- **Photos are downscaled at capture** to 2560px on the long edge (`downscaleForStorage` in `photo-service.ts`). Phones hand over 4080x3060 / 12.5MP / 3-9MB each; a 14-photo receipt was moving ~120MB off the phone because every file goes to SharePoint **and** Azure Blob. 2560px keeps ~5MP — more than any current Claude model reads and plenty to zoom a nameplate — for roughly a quarter of the bytes.
- `createImageBitmap` is called with `imageOrientation: "from-image"`. Phone cameras record rotation in EXIF rather than rotating pixels, and a canvas redraw that ignores it stores every portrait photo sideways.
- Vision calls resize to **1568px** — the ceiling for `claude-haiku-4-5`, which downsamples anything larger. Sending 1800 was uploading pixels the model discarded. High-resolution vision (2576px, ~4757 tokens/image vs 1580) exists only on Opus 4.7+ and Sonnet 5, so raising it means changing model — measured at roughly $10/month more at current volume, for no benefit until something actually reads nameplates.

## Azure Blob — the warehouse copy
- Every receiving photo goes to **SharePoint and Azure Blob**, not one or the other. SharePoint stays where the warehouse works from; Blob is the copy anything else can read. **A SharePoint file is fetched by signing in AS THE PERSON who uploaded it** — so no script, report or future tool can reach one. That is not theoretical: 282 photos of known parts could not be read for a valve-recognition measurement, because the only credentials available hold `Mail.Send` and nothing more. Granting more needs a Microsoft 365 administrator.
- The phone uploads **straight to Azure**. The proxy only mints a permission — `createUploadPermission` in `proxy/src/services/blob-storage.ts` — valid 15 minutes, create-and-write only, one named blob, container allowlisted. It cannot read, list or delete. Photos never pass through the proxy: a receipt is a dozen files of several megabytes and routing them through would slow the receiver for nothing.
- **Azure refuses a browser without a CORS rule, and the failure is invisible.** The first release copied nothing for a day: every upload was blocked at the preflight, before a byte was sent. The test that "proved" it worked ran from Node, which ignores cross-origin rules entirely — it proved the permission worked and never proved a *browser* could use it. The rule allows PUT from `https://dgoelztork.github.io` and localhost only; anything else gets 403. Check it with an `OPTIONS` carrying `Origin` and `Access-Control-Request-Method: PUT`.
- `file_cards.blob_container` is set **only when a copy actually landed**, and left null otherwise, so a later backfill can find what is still unreachable rather than assuming. `sha256` is taken in the browser while the file is in hand — fingerprinting later would mean reading every photo back out of storage.
- **Registering a user table as an object moves it in the Service Layer** (`/U_NAME` becomes `/NAME`, and the old address stops existing). Unrelated to this app, but the same class of surprise: anything reading the old path breaks silently.
...
- **Neither site's Zebra is reachable from TORK-APP**, so both go through `print-agent/` — a PowerShell agent on the PC the printer is plugged into. It listens on 9100 and hands raw ZPL to the Windows spooler, making a USB printer look like a network one. The proxy is unchanged: `host` is just the PC's Tailscale address. Alameda's Zebra never appeared on `192.168.201.0/24` (a full scan found one Brother), and Pascagoula has no link to the server at all — this avoids cabling and VLAN work at both.
- The agent must print **RAW**. ZPL is control code, not a document; anything that renders it produces pages of gibberish instead of a label. It reaches the spooler through `winspool.drv` P/Invoke because no managed API sends bytes untouched.
- PowerShell scripts in `print-agent/` are saved **UTF-8 with BOM**. Windows PowerShell 5.1 reads a BOM-less file as ANSI, which mangles every non-ASCII character and breaks string parsing — the em-dashes in the comments are enough to stop the script loading.
- Its firewall rule is scoped to `100.64.0.0/10`, the Tailscale range, so only tailnet machines can print — not the warehouse network, not the internet.

## Feedback
- Floating button (`src/components/FeedbackButton.tsx`) mounted inside the router in `App.tsx`, so a receiver can file a bug or idea from any screen and the submission records which one. The list lives on its own screen (`src/screens/FeedbackBoard.tsx`, `/feedback`), reachable from the Dashboard.
- **Self-contained — nothing to do with Scupper.** An earlier version posted into Scupper's board so there'd be one list; Dylan wants the apps separate, so this owns its own data and touches nothing else.
- Storage is a local SQLite file via `node:sqlite` (built into Node, no dependency), default `proxy/data/feedback.db`, overridable with `FEEDBACK_DB_PATH`. The table is created on first use, so there's no migration step to forget. Gitignored.
- Why not the existing databases: the proxy's Azure SQL login can write `dbo.file_cards` but is **denied CREATE TABLE** on `torkdata`, and a Postgres database needs credentials this service doesn't have. If an Azure SQL table is created later, only `feedback-store.ts` changes — the route and UI don't care where rows live.
- Scupper auto-captures the screen with html2canvas; this app offers **the camera** instead. It's a phone in a warehouse — the useful picture is the part or the shelf, and screen capture is unreliable inside an iOS web app.
...
- Photos are stored as data URLs but **never returned by the list endpoint** — it carries a `hasPhoto` flag and the image is fetched per row from `GET /api/feedback/:id/photo`, so one screen doesn't drag down megabytes. That endpoint needs the auth header, so the client fetches it as a blob rather than using a plain `<img src>`.
- **No notification email.** The proxy has no Graph client secret and the PWA's MSAL scopes are `Files.ReadWrite.All` + `User.Read` — no `Mail.Send`. Emailing on a new submission needs an Azure app-registration change; until then feedback is read on the board.
...
- Photo bytes are pruned only once `session.photosUploaded === true` (`keeper()` in the store) — i.e. when the upload is confirmed clean. Pruning at `SUBMITTED` alone was too eager: a receipt whose upload died partway is precisely the one still holding the only copy. Confirmed-good receipts stop carrying ~10MB each on phones already short of memory.
- A submitted receipt whose photos didn't land shows **Upload the photos** in `ReviewSubmit`, which re-runs the same upload path and stamps `U_GRPODocs`. `session.grpoDocEntry` is persisted for exactly this — the stamp needs the DocEntry, and local state is gone after a reload. The prompt is gated on `hasPhotoBytes()`, so receipts from before photos persisted (empty blobs) don't offer a button that can only fail.
